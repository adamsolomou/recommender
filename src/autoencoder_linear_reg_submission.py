...
for _ in range(10):
    ...
    model3.train(data_zeros, data_mask, n_epochs=200, verbose=False)
    ...
    list_preds_train.append(preds_train.reshape(-1,1))
    list_preds_test.append(preds_test.reshape(-1,1))

predictions_train = np.concatenate(list_preds_train, axis=1)
predictions_test = np.concatenate(list_preds_test, axis=1)

from sklearn.linear_model import LinearRegression

model = LinearRegression()
model.fit(predictions_train, ratings_train)
ratings_test_pred = model.predict(predictions_test)

submission['Prediction'] = ratings_test_pred
submission.to_csv('autoencoder_lin_reg_preds.csv', index=False)
